...
| `etl_anp_vendas.yml` | **Cron interno `7,37 * * * *` (a cada 30 min, PRIMÁRIO desde 2026-06-10)** — cadência internalizada; a dependência do cron-job.org foi **aposentada** para este workflow (o job externo morreu em 2026-06-01 e não pode ser re-armado). `workflow_dispatch` permanece disponível (manual + eventual trigger externo futuro, hoje não configurado). Offset :07/:37 evita a congestão de top-of-hour do GHA; cron GHA é best-effort (jitter de minutos, skips ocasionais) — aceitável porque o watcher é no-op idempotente ~95% do tempo, o dado é mensal e o silence detector (26h) faz backstop. Concurrency group `anp-vendas` (`cancel-in-progress: false`) serializa runs empilhados por jitter × dispatch manual. | `pipelines/anp/vendas_watch.py --force` | `vendas`. Semântica de upload (idempotente sob re-runs frequentes): período estritamente mais novo → upload só daquele mês; **mesmo período com MAIS rows na fonte que no DB → refresh in-place do mês** (delete+insert; mês parcial cresce até completo, nunca encolhe, nunca toca histórico — adicionado 2026-06-10, ver § "ANP Vendas — dispatcher death & frozen partial month"); mesmo período, mesmas/menos rows → no-op. |
...
**RESOLVED — incident closed 2026-06-10:** cadence internalized — the internal GHA cron was promoted to PRIMARY at `7,37 * * * *` (every 30 min, replacing the 2h fallback `17 */2 * * *`); the cron-job.org vendas job is left dead **intentionally** (the console is logged out and no API key exists — it cannot be re-armed), so the "pending user action" above is void and **no user action is pending anymore**. `workflow_dispatch` remains for manual/future external triggers (none configured).
