...
| `stock_guide_sensitivities` | Tabela nova (`20260606000000`). Tabelas de sensibilidade first-class. PK `id bigint GENERATED ALWAYS AS IDENTITY`. Colunas: `title`, `value_mode text CHECK IN ('absolute','yield','pe','ev_ebitda','upside')`, `metric_label`, `unit`, `companies text[]` (tickers que a tabela envolve — dirige drill-down + hide gating), `definition jsonb` (object: dois eixos `row_axis`/`col_axis` cada um `kind=driver|company|year` — driver carrega `driver_id` + `scenarios[]` per-table; company carrega `companies[]`; year carrega `years[]` — + `cells[][]` primário + `cells_secondary[][]` opcional só para `ev_ebitda`), `display_order`, `updated_at`, `updated_by`. RLS habilitada, zero policies. Cobre cross-company, single-company 2D (driver×driver), por-ano e 1D. **Bloco opcional `definition.compose` (elástica, `20260611000000`):** presente quando a tabela é "elástica" (coeficiente/elasticidade) — `{ output:"target_price", driver_keys[], anchors{driver_key→nível}, base{ticker→TP BRL/ação na âncora}, by_company{ticker→{driver_key→slope}}, scenarios{nome→{driver_key→nível}} }`. `definition.compose != null` marca a tabela como elástica; o browser compõe Target Price ao vivo (`TP = base + Σ slope×(nível−âncora)`) + Upside. Gravado **verbatim** pelo upsert. Ver contrato de hide-strip abaixo. |
...
Helpers internos `_sg_filter_rows(jsonb,text,int[])` / `_sg_filter_cols(jsonb,text,int[])` / `_sg_strip_compose(jsonb,text[])` (`IMMUTABLE SECURITY DEFINER`, **não** granted a anon/authenticated — chamados só de `get_stock_guide_sensitivity_tables`; `_sg_strip_compose` adicionado em `20260611000000` para o hide-strip dos mapas elásticos per-ticker). As 3 RPCs antigas de grade 1:1 (`get_stock_guide_sensitivity`, `admin_get_stock_guide_sensitivity`, `admin_upsert_stock_guide_sensitivity`) ficam **dormentes** pendente cleanup.
...
**Sensibilidade elástica (coeficiente, `20260611000000`):** uma tabela de sensibilidade vira **elástica** quando seu `definition` carrega um bloco `compose` (shape no row de `stock_guide_sensitivities` acima). O dashboard então renderiza um painel de sliders contínuos (Brent + FX por ano: 2026/2027/**2028**) que re-precifica **target price + upside** ao vivo no browser (`composeElasticTargetPrice` em `src/lib/stockGuideSensitivity.ts`: `TP[c] = base[c] + Σ_k by_company[c][k] × (nível[k] − anchors[k])`; key ausente → contribui zero; nível ausente → cai na âncora; nunca `NaN`; upside = `TP/live − 1`). **Contrato de segurança (cross-dept):** o RPC `get_stock_guide_sensitivity_tables` chama `_sg_strip_compose(def, visible[])` para não-admin, removendo de `compose.base` e `compose.by_company` os tickers ocultos (os dois únicos campos per-ticker sensíveis) — `output`/`driver_keys`/`anchors`/`scenarios` passam intactos. Ticker ausente de `compose.base` ⇒ nenhuma row renderizada para ele (o frontend itera só `Object.keys(compose.base)`). O upsert `admin_upsert_stock_guide_sensitivity_table` grava `compose` verbatim. Editor admin: o builder de sensibilidades ganhou o modo "Elastic (coefficient)" (force `value_mode='upside'`). Detalhe completo (sliders, presets Live/Base/Bull/Bear/Custom, bounds, UI dual-view): [`docs/app/stock-guide.md`](app/stock-guide.md) § "Elastic (coefficient) sensitivity".
