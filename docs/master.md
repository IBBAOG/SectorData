...
| `stock_guide_sensitivities` | Tabela nova (`20260606000000`). Tabelas de sensibilidade first-class. PK `id bigint GENERATED ALWAYS AS IDENTITY`. Colunas: `title`, `value_mode text CHECK IN ('absolute','yield','pe','ev_ebitda','upside')`, `metric_label`, `unit`, `companies text[]` (tickers que a tabela envolve — dirige drill-down + hide gating), `definition jsonb` (object: dois eixos `row_axis`/`col_axis` cada um `kind=driver|company|year` — driver carrega `driver_id` + `scenarios[]` per-table; company carrega `companies[]`; year carrega `years[]` — + `cells[][]` primário + `cells_secondary[][]` opcional só para `ev_ebitda`), `display_order`, `updated_at`, `updated_by`. RLS habilitada, zero policies. Cobre cross-company, single-company 2D (driver×driver), por-ano e 1D. **Bloco opcional `definition.grid` (malha de cenários multi-eixo, multi-eixo desde `20260618200000`):** metadado de eixo **não-sensível** — `{ axes: [{driver_key, label, unit}] (1..3, ordem = x,y,z), output:"target_price" }` — que **não nomeia empresa** (logo dispensa hide-strip; gravado **verbatim** pelo upsert). Marca a tabela como uma malha de Brent interpolada multilinearmente (1-D vira caso degenerado); os valores por papel `(coords de Brent → target price)` ficam na tabela relacional separada `stock_guide_scenario_grid` (não no jsonb), lidos via `get_stock_guide_scenario_grid`. O shape legado 1-D `{ x_driver_key, x_label, x_unit, output }` foi convertido para o bloco `axes` pela migration `20260618200000`. **Substituiu o bloco linear `definition.compose` (elástica, `20260611000000`)** — o `compose` + o helper `_sg_strip_compose` continuam **dormentes** no banco (não dropados), mas o frontend não os renderiza mais. |
| `stock_guide_scenario_grid` | Tabela nova (1-D `20260612000000`, multi-eixo `20260618200000`). Malha de cenários multi-eixo (1..3 eixos) de Brent. PK `(sensitivity_id bigint, ticker text, x_value numeric, y_value numeric, z_value numeric)`. `sensitivity_id` FK→`stock_guide_sensitivities` `ON DELETE CASCADE`. `x_value`/`y_value`/`z_value` = níveis de Brent por eixo (US$/bbl; ordem = ordem dos `definition.grid.axes`); `y_value`/`z_value` são `numeric NOT NULL DEFAULT 0` (eixo não usado = 0 permanente — 1-D grava `(sid, ticker, x, 0, 0)`, 2-D `(…, x, y, 0)`, 3-D `(…, x, y, z)`). `primary_value numeric` = target price (R$/ação) naquele cenário. Uma row por `(tabela de sensibilidade, ticker, combinação de coordenadas)` — o frontend lê a malha por ticker e **interpola multilinearmente** (2^d cantos) contra os níveis de Brent ao vivo; Upside = `TP / preço_live − 1`. RLS habilitada, **zero policies** — leituras só via RPC `get_stock_guide_scenario_grid(p_sensitivity_id)` (SECURITY DEFINER, hide-aware); escritas só via service role (`scripts/manual/stock_guide_brent_grid_upload.py`, formato LONG Cartesiano-completo, replace-total por `sensitivity_id`, bypassa RLS). A migration `20260618200000` foi **ALTER** (não DROP) — adicionou `y_value`/`z_value` + trocou o PK 3→5-col, preservando RLS/ownership/grants; o DEFAULT 0 backfilla as rows pré-existentes. O PK de 5 colunas já cobre o read pattern (filtro por `sensitivity_id` + ordenação por `ticker, x_value, y_value, z_value`) — sem índice extra. |
...
|---|---|---|
| `get_stock_guide_comps` | `() → TABLE(ticker, company_name, is_visible, display_order, sector, volume_unit, yahoo_symbol, shares_outstanding, net_debt_y1, net_debt_y2, last_update, target_price, recommendation, ebitda_y1/y2, net_income_y1/y2, net_income_adj_y1/y2, fcfe_y1/y2, dividends_y1/y2, volumes_y1/y2)` (fundamentos, não múltiplos — os 4 múltiplos são derivados no browser; `net_income_adj_y1/y2` adicionados em `20260610000000`, hide-aware, logo após `net_income_y2`). **Hide-aware:** para linha oculta vista por não-admin, força a NULL todos os campos exceto `ticker`/`company_name`/`is_visible`/`display_order` (inclui `yahoo_symbol` + os fundamentos — restrito nunca chega ao browser, então o mkt cap também não vaza). Admin recebe tudo. | anon + authenticated |
...
| `admin_get_stock_guide_companies` | `() → TABLE(todas as colunas incl. ocultas — fundamentos `net_debt_y1`/`net_debt_y2`/`net_income_y1/y2`/`net_income_adj_y1/y2`/`fcfe_y1/y2`/`dividends_y1/y2`/`ebitda_y1/y2`/`volumes_y1/y2` + `shares_outstanding` + `updated_at` + `updated_by`; sem múltiplos armazenados)` | authenticated, `is_admin()` |
| `admin_get_stock_guide_sensitivity` | `(p_ticker text) → jsonb` | authenticated, `is_admin()` |
| `admin_upsert_stock_guide_company` | `(p_ticker text, p_data jsonb) → void` (lê as chaves de fundamentos `net_debt_y1`/`net_debt_y2`/`net_income_y1/y2`/`net_income_adj_y1/y2` (opcionais)/`fcfe_y1/y2`/`dividends_y1/y2` em `p_data`; não lê mais chaves de múltiplos) | authenticated, `is_admin()` |
...
| `get_stock_guide_scenario_grid` | `(p_sensitivity_id bigint) → TABLE(ticker, x_value, y_value, z_value, primary_value)` ordenado por `ticker, x_value, y_value, z_value` (1-D `20260612000000`, recriada com 5 colunas em `20260618200000`). Malha multi-eixo (1..3) de Brent → target price de uma tabela de sensibilidade. **Hide-aware:** para não-admin, só retorna tickers `is_visible` em `stock_guide_companies` (`is_admin() OR EXISTS visible` — mesmo critério de `get_stock_guide_sensitivity_tables`); os níveis de uma empresa oculta nunca chegam ao browser. Recriada via **DROP+CREATE** em `20260618200000` (a `RETURNS TABLE` mudou de 3→5 colunas) com SECURITY DEFINER + `SET search_path = public, pg_temp` + GRANT anon/authenticated re-assertados (pegadinha #18). | anon + authenticated |
...
**Sensibilidade por malha de cenários multi-eixo de Brent (1-D `20260612000000`, multi-eixo `20260618200000`, atual):** uma tabela de sensibilidade vira uma **malha de Brent interpolada** quando seu `definition` carrega um bloco `grid` (`{ axes: [{driver_key, label, unit}] (1..3, ordem = x,y,z), output:"target_price" }` — metadado de eixo, **sem empresa**, gravado verbatim). Os eixos da v1 são Avg Brent 2026 / Avg Brent 2027 / Brent 2028+ (1-D vira caso degenerado). Os valores por papel **não** ficam no jsonb: ficam na tabela relacional `stock_guide_scenario_grid` (uma row por `(sensitivity_id, ticker, x_value, y_value, z_value)` com `primary_value` = target price; eixo não usado = 0). O dashboard renderiza **até 3 sliders de Brent** (um por eixo, com marcador "live" por ano) e, ao vivo no browser, faz **interpolação multilinear** (2^d cantos) → target price; **Upside = `TP / preço_live − 1`**. Helpers `buildGridMesh` / `interpolateMesh` em `src/lib/stockGuideSensitivity.ts` (substituíram `interpolateGrid` 1-D; cobertos por testes vitest). Substitui o painel elástico anterior. **Contrato de segurança (cross-dept):** os valores por papel são lidos pelo RPC dedicado `get_stock_guide_scenario_grid(p_sensitivity_id)` (SECURITY DEFINER, hide-aware — só tickers `is_visible` para não-admin), não pelo jsonb; níveis de empresa oculta nunca chegam ao browser. O bloco `grid` não nomeia empresa → dispensa hide-strip.

**Fluxo de upload (Dados Locais):** o analista roda o modelo dele em **todas as combinações Cartesianas** dos eixos e grava um Excel no formato **LONG** — 1 linha por cenário, com colunas de coordenada **nomeadas pelos driver_keys** dos eixos (`avg_brent_2026`, `avg_brent_2027`, `avg_brent_2028`) + 1 coluna por ticker = target price. O loader `scripts/manual/stock_guide_brent_grid_upload.py` (owner: `worker_dados-locais`) deriva as colunas de coordenada de `grid.axes`, valida completude Cartesiana (`len == Π níveis`), grava `x/y/z_value` (eixo não usado = 0) e faz **replace-total por `sensitivity_id`** (apaga todas as rows do alvo e reinsere; idempotente; service role bypassa RLS — a regra "nunca deletar mês parcial" **não se aplica**, é snapshot). O template é gerado por `scripts/manual/make_brent_grid_template.py` (produto Cartesiano via `--range key=min:max:step`). **Guidance de payload:** ≤15 níveis/eixo em 3-D (≈ 27k rows), ≤40×40 em 2-D, livre em 1-D. A "casca" (shell) que recebe a malha é uma linha em `stock_guide_sensitivities` marcada por `definition.grid`, criada pelo Admin Panel. Detalhe completo (sliders, interpolação, presets, UI dual-view): [`docs/app/stock-guide.md`](app/stock-guide.md); detalhe do loader: [`docs/dados-locais/PRD.md`](dados-locais/PRD.md).
...
**Monitoring & testing (ops, 2026-06):** dois monitores dão cobertura total da frota de ETL — `freshness_monitor.yml` (guardião de frescor: alerta quando uma base para de avançar mesmo com workflow verde — *stall silencioso*) + `workflow_failure_monitor.yml` (pager de falha: alerta em ≥3 falhas consecutivas não-canceladas de 16 workflows críticos — *falha barulhenta*; re-home do `etl_workflow_stuck`). Mais `client_alerts_poll.yml` (poll a cada 20 min para bases Data Input sem hook) e `client_alerts_test.yml` (harness `run_base --test` production-safe). O monitor legado `alertas_monitor.yml` está **DESABILITADO** (reversível) e os 3 destinatários internos foram migrados para o Client Alerts. Detalhes em `docs/etl-pipelines/PRD.md` § "Monitoring & testing" / "Legacy `alertas/` monitor retirement".
