...
| `stock_guide_drivers` | Tabela nova (`20260606000000`). Registry central de variáveis macro/assumption (Brent, USD/BRL, etc.) — **não** company-sensitive. PK `id bigint GENERATED ALWAYS AS IDENTITY`. Colunas: `name` (ex.: `Brent average 2026E`), `unit` (ex.: `USD/bbl`), `current_value numeric` (o valor "current/base" exibido nas tabelas de sensibilidade), `display_order`, `updated_at`, `updated_by`. RLS habilitada, zero policies (não-sensível, mas acesso só via RPC SECURITY DEFINER). |
...
| `admin_get_stock_guide_companies` | `() → TABLE(todas as colunas incl. ocultas — fundamentos `net_debt_y1`/`net_debt_y2`/`net_income_y1/y2`/`fcfe_y1/y2`/`dividends_y1/y2`/`ebitda_y1/y2`/`volumes_y1/y2` + `shares_outstanding` + `updated_at` + `updated_by`; sem múltiplos armazenados)` | authenticated, `is_admin()` |
| `admin_get_stock_guide_sensitivity` | `(p_ticker text) → jsonb` | authenticated, `is_admin()` |
| `admin_upsert_stock_guide_company` | `(p_ticker text, p_data jsonb) → void` (lê as chaves de fundamentos `net_debt_y1`/`net_debt_y2`/`net_income_y1/y2`/`fcfe_y1/y2`/`dividends_y1/y2` em `p_data`; não lê mais chaves de múltiplos) | authenticated, `is_admin()` |
...
| RPC | Assinatura / Comportamento | Acesso |
|---|---|---|
| `get_stock_guide_drivers` | `() → TABLE(id, name, unit, current_value, display_order)` — assumptions macro, não-sensíveis (retorno completo a todos) | anon + authenticated |
...
| `admin_upsert_stock_guide_driver` | `(p_id bigint, p_data jsonb) → bigint` (NULL `p_id` → INSERT, senão UPDATE; retorna id) | authenticated, `is_admin()` |
