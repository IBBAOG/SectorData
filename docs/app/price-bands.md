...
- X axis: the previous strategy extended `xaxis.range` by 18 % of the visible window beyond the last data point, which pushed the right edge ~2 months into the future at 1Y zoom (May 2026 last point → Jul-26 axis end). Replaced by a **pixel-based gutter**: `margin.r = 70` + `xaxis.automargin: true` reserves fixed room for tick labels and annotations without distorting the data range. Traces set `cliponaxis: false` defensively so the annotations themselves can still render in the margin gutter.
