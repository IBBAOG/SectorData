...
| `get_anp_cdp_depletion_scatter(p_campos text[], p_expand_canonical bool DEFAULT false)` | own (per-well view) | Returns one row per (well × month) for the filtered fields, with server-computed `np_kbpd` (uptime-normalized daily flow, kbpd) and `mes_desde_t0`. RETURNS TABLE. Capped at 500k points server-side. **`p_expand_canonical` (default `false`):** when `true`, expands `p_campos` via the `canonical_field_name()` helper (Round 4 of `/well-by-well`) — every field whose canonical matches any input is included (e.g. `p_campos=['TUPI']` aggregates `{TUPI, SUL DE TUPI, AnC_TUPI}`). Dashboard `/anp-cdp-depletion` always uses the default `false` — standalone behavior is unchanged. |
| `get_anp_cdp_depletion_field_aggregate(p_campos text[], p_expand_canonical bool DEFAULT false)` | own (field-average view) | Returns one row per (field × calendar month) with `np_kbpd` (field-aggregate kbpd), `n_pocos`, `pct_voip` (cumulative oil ÷ VOIP), and `cumulative_oil_bbl`. RETURNS jsonb (single row, single column) to bypass PostgREST default `max_rows=1000`. Same `p_expand_canonical` semantics as the scatter variant — `/anp-cdp-depletion` uses default `false`; `/well-by-well` drill-down popup uses `true` via the wrapper `rpcGetAnpCdpDepletionFieldAggregateCanonical`. |

> **Cross-dashboard usage (Phase 2 of `/well-by-well` drill-down, 2026-05-30):** the 2 Depletion RPCs are now also consumed by the Depletion tab of the field drill-down modal in `/well-by-well`, through canonical-aware wrappers in `src/lib/rpc.ts` (`rpcGetAnpCdpDepletionScatterCanonical` / `rpcGetAnpCdpDepletionFieldAggregateCanonical`) that pass `p_expand_canonical=true`. The chart builders `buildPerWellChart` and `buildFieldAverageChart` were extracted into [`src/lib/charts/depletion.ts`](../../src/lib/charts/depletion.ts) so both dashboards render with identical visual logic — any change to the builders affects both call sites. Important difference vs the standalone dashboard: the `/well-by-well` popup pins the X axis to **% VOIP recovered** (the Calendar/VOIP toggle of the standalone dashboard is not exposed there). Migration: [`supabase/migrations/20260530000000_cdp_rpcs_canonical_expansion.sql`](../../supabase/migrations/20260530000000_cdp_rpcs_canonical_expansion.sql).
