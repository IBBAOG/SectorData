...
**YoY tables** follow the canonical pattern shared with `/anp-cdp-bsw` and `/anp-cdp-depletion`: scrollable container (`maxHeight: 400`, `overflowY/X: auto`, `minWidth: 540`, `1px solid #ececec`, `borderRadius: 4`) wrapping a Bootstrap `table table-sm table-striped` with `Arial 12px`, `table-layout: fixed`, and a `<colgroup>` that pins column widths (Entity 28%, five value cols 14.4% each) so all three YoY instances (Panel A, Panel B, Exports) render identically regardless of unit label length (`kt` vs `mil m³`). 2026-05-27: standardized YoY table widths across Panel A / Panel B / Exports. The `<thead>` is sticky (`position: sticky; top: 0; background: #fff`) and each `<th>` carries a darker bottom border (`2px solid #888`). The first column ("Entity") prepends an 8×8 color dot matching the chart's palette via `colourForEntity(entities, entity)` and is ellipsis-truncated at `maxWidth: 220` with a `title` tooltip. All numeric cells use `fontVariantNumeric: "tabular-nums"`. Delta cells (YoY %) render via `fmtDelta`: `#28a745` for positive, `#dc3545` for negative, `#666` for null/zero/non-finite (formatted as `—`), `fontWeight: 600`. The prior-period cell is muted (`#777`). The same `<YoYTable>` component instance is reused across all panels (Imports by Origin Country, Imports by Importer, Exports by Destination Country) — no per-panel forks.
...
- **Server-side 'Others' is filtered BEFORE client ranking.** The RPC `get_imports_exports_importers_stacked` already collapses rank-≥(top_n+1) into a row labeled `Others`. Letting that row enter the client-side ranking would put two distinct entities both labeled "Others" in the chart legend (one from the server collapse, one synthesized client-side) and would also let the server's pre-aggregated "Others" volume earn a top-6 slot it does not deserve. Fix: partition `importersData` into named rows vs. server-`Others` rows first, then rank named only.
- **Chart**: top-6 named importers ranked by `SUM(total_mil_m3)` over the full selected window. The client-side "Others" series sums BOTH the rank-≥7 named importers AND the server-side `Others` rows per (ano, mes). Volume is preserved — nothing is discarded.
- **YoY table**: top-6 entity order is **inherited from the chart's `importersTop6Entities`** (same window-sum ranking). The table then looks up Latest / MoM% / YoY% values for those entities from `yoyImportersData` (which carries anchor-month single-month semantics since migration `20260527000000`). Server-side `Others` rows + named importers outside the top-6 are folded into the table's aggregated `Others` row; `yoy_pct` is recomputed on the aggregated totals. If the chart picks a top-6 importer for which the YoY RPC returned no row (rare — the chart sees the full window, the YoY RPC anchors on a single month), the table renders a zero row in that slot so the ordering stays in lockstep with the chart legend.

Chart and table now always share the same 7-entity set in the same order; an importer can never appear in one and be absent from the other. The two rankings come from the same window-sum derivation; the YoY RPC only contributes the per-cell values, not the ranking.

Implementation:

- Hook `useImportsExportsData` exposes three new outputs: `importersTop6Data` (chart rows), `importersTop6Entities` (canonical order: top-1 → top-6 → Others), `importersTop6ColorMap`, plus `yoyImportersTop6Data` (7-row max — depends on `importersTop6Entities` so the table inherits the chart's ranking).
- Server RPC unchanged — `get_imports_exports_importers_stacked` still returns up to top-10 by server-side ranking; client filters out the server-`Others` row, re-ranks the remaining named importers by window sum, and collapses to 6 + Others.
