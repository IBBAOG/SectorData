...
## Elastic (coefficient) sensitivity — multi-year Brent/FX sliders

Source of truth: `supabase/migrations/20260611000000_stock_guide_sensitivity_compose_hide_strip.sql` (DB hide-strip helper, owner `worker_supabase`, applied live) + the frontend below. A sensitivity table is **ELASTIC** when its `definition` carries a **`compose`** block; the dashboard then renders a continuous-slider panel that re-prices the **target price + upside** live as the analyst drags Brent / FX by year (2026 / 2027 / 2028), instead of the static matrix. Static tables (no `compose`) are unchanged.

### The `compose` block (stored verbatim by the upsert RPC)

```
definition.compose = {
  output:      "target_price",                       // BRL/share (currently the only output)
  driver_keys: ["avg_brent_2026", … , "avg_fx_2028"],// catalog keys this table composes against
  anchors:     { "avg_brent_2026": 80, "avg_fx_2026": 5.2, … },   // driver level the base was measured at, per key
  base:        { "PETR4": 42.0, "PRIO3": 48.0, … },              // base target price (BRL/share) at the anchors — PER TICKER
  by_company:  { "PETR4": { "avg_brent_2026": 0.15, "avg_fx_2026": 3.0, … }, … }, // slope Δ(TP)/Δ(driver) — PER TICKER
  scenarios:   { "Base": {"avg_brent_2026":80,…}, "Bull": {…}, "Bear": {…} }      // optional named presets (levels per key)
}
```

**Composition (browser, pure):** `composeElasticTargetPrice(ticker, driverLevels, compose)` in `src/lib/stockGuideSensitivity.ts` →
`TP[c] = base[c] + Σ_k by_company[c][k] × (level[k] − anchors[k])`. A driver key absent from `by_company[c]` contributes zero; a missing `level[k]` falls back to the anchor (so the output sits at base for that key); non-finite inputs are skipped → never `NaN`. **Upside** = `TP / livePrice − 1` (guard `livePrice > 0`), reusing the dashboard's `liveByTicker` price index.

**Hide-strip rule (critical):** the read RPC's `_sg_strip_compose(def, visible[])` removes every ticker key NOT visible from `compose.base` and `compose.by_company` (anchors / driver_keys / scenarios / output name no company, so they pass through). **If a ticker is absent from `compose.base`, no row is rendered for it** — `composeElasticTargetPrice` returns `null`, and the hook only iterates `Object.keys(compose.base)`. Never assume every `companies[]` entry has a base.

### Catalog 2028 (`src/hooks/useMarketDrivers.ts`)

`MARKET_DRIVER_CATALOG` grew from 4 to **6 dynamic metrics**: `avg_brent_2026/2027/2028` (USD/bbl) + `avg_fx_2026/2027/2028` (BRL/USD). `avgBrentForYear` / `avgFxForYear` already accept any forward year: **Brent 2028** uses the third forward window; if the forward curve doesn't reach 2028 (typical), every month falls back to spot → a **spot-flat approximation** (documented). **FX 2028** = spot-flat (no FX forward in the proxy, same as 2027). `resolveDriverValue` resolves all six keys; the four legacy keys are unchanged (back-compatible).

### Hook (`useStockGuideData.ts`)

The shared brain owns the elastic state so **both Views drag the same sliders**:

- `isElasticTable(table)` — true when `definition.compose != null`.
- Per-table state `{ levels: { driver_key → number }, preset }` (`elasticState`). Each level **initializes to the LIVE market value** for that key (`resolveDriverValue` via the catalog), fallback the anchor. A table tracking "Live" re-seeds from the market once the values land (without clobbering a manual drag).
- `getElasticModel(table) → { sliders, presetNames, preset, rows, outputLabel }` (or null for a non-elastic table). `sliders` are one per driver_key, grouped Brent→FX→other and year-ascending, each carrying `{ label, type, year, unit, anchor, liveValue, level, min, max, step }` (bounds = span of {anchor, live, every scenario level, current level} padded ~15 % with a tidy 1/2/5×10ⁿ step). `rows` = one per visible ticker **with a base**, `{ ticker, companyName, targetPrice (composed), upside, basePrice, livePrice }`, sorted by company display order.
- `setElasticDriverLevel(tableId, key, level)` — sets one slider → enters preset **"Custom"**. `setElasticPreset(tableId, preset)` — "Live" resets to market values; a scenario name sets levels from `compose.scenarios[name]` (fallback anchor per key).

### Elastic panel UI (both Views, dual-view binding honored)

When a `selectedTable` is elastic, the View renders an **elastic panel** in place of the static matrix:

- **Sliders** grouped "Brent (USD/bbl)" / "FX (USD/BRL)", three years each, every slider initialized at its live value and showing the anchor + live caption; the live value re-prices on load.
- **Scenario selector** — `Live` + the named presets that exist in `compose.scenarios` (Base / Bull / Bear), plus an implicit `Custom` chip once a slider is dragged.
- **Output table** — rows = companies with a base; columns = **Target price** (composed) and **Upside** (vs live price, coloured by sign, "—" while quotes load); desktop adds a **vs base** Δ column. Copy is English ("Drag Brent / FX by year to re-price live", "Target price", "Upside", "Scenario").
- **[mobile-only] divergence:** sliders stack vertically and the output renders as a stacked Company / Target price / Upside list (no Δ column) — same shared brain, adapted layout.

### Admin elastic editor (admin-panel, desktop-only)

The Sensitivities builder gained an **"Elastic (coefficient)"** mode toggle (vs "Static matrix"). In elastic mode the axis/cell-matrix editor is replaced by the compose form: **Output** select (target price), **Drivers** checkboxes (the 6-metric catalog incl. the three Brent + three FX years), **Anchors** per driver_key, a **Companies** multi-select, the **base + slope grid** (companies × driver_keys, "Base TP" highlighted; helper text: "slope = Δ target price per +1 unit of the driver"), and optional **named scenarios** (Base/Bull/Bear → levels per key). Toggling elastic forces `value_mode='upside'`. Validation mirrors the dashboard contract (≥1 driver, ≥1 company, numeric anchors/base/slopes, scenario names + numeric levels; `base` and `by_company` over the same companies; driver_keys consistent). On save the hook serializes everything into `definition.compose` (verbatim) inside `p_data` of `admin_upsert_stock_guide_sensitivity_table`; editing an existing elastic table prefills from `admin_get_…`'s full `definition`. The list badge reads **Elastic** for these tables. `companies[]` is derived from the base keys.
