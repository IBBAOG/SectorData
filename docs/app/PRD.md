...
| `anp-cdp-diaria` | Oil & Gas | `anp_cdp_diaria` (~16.5k — Field), `anp_cdp_diaria_instalacao` (~16.3k — Installation), `anp_cdp_diaria_poco` (~180.7k — Well) + Company tier (`anp_cdp_diaria` × `field_stakes`; since `20260618000000` the effective stake is a per-(canonical field, month) blend read from `mv_production_monthly` — see `docs/app/anp-cdp-diaria.md` § "Blended effective stakes"). Histórico desde 2025-11-09. |
