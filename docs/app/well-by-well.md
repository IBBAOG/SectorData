...
**Pagination (P0 fix, 2026-05-29; follow-up same day).** The full-history RPCs accept `p_offset` + `p_limit` and the export spec loops in **`PAGE_SIZE = 50000`** chunks. The loop breaks **only on an empty chunk** — a partial chunk is NOT a reliable end-of-data signal, because exactly-`PAGE_SIZE` rows remaining come back as a "full" page and only the next call returns empty. Requires PostgREST `db_max_rows >= 50000` (raised from the default 1000 by `worker_supabase` in parallel); without that, the server silently truncates every page to the cap and the export is incomplete. The earlier short-chunk heuristic (`chunk.length < PAGE_SIZE` → break) misfired catastrophically while the cap was still 1000: every chunk was capped at 1000, so the check tripped after page 1 and the export shipped with only 1000 rows per sheet. With ~2.2M Brasil rows, ~45 round-trips at ~500ms each ≈ ~22s for a full-history export — acceptable for a Tier 2 modal-gated download. A hard offset safety cap (`MAX_OFFSET_SAFETY = 5_000_000`) guards against runaway loops. Each sheet's `rowsAsync` is wrapped in try/catch so a single sheet failure (e.g. one company RPC errors out) emits an empty rowset instead of blanking the entire workbook.
