...
### Wave 5 — `/profile` (completed 2026-05-20) and `/admin-panel` (parallel worktree)

`/profile` is now a full dual-view module. `/admin-panel` is being refactored in a parallel worktree as part of the same Wave 5 batch.

#### `/profile` dual-view (completed 2026-05-20)
...
#### `/admin-panel` dual-view — parallel worktree

`/admin-panel` is being refactored in a parallel worktree as part of Wave 5. Until that lands, the route renders desktop-only.
