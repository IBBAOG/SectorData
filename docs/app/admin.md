...
- **Validation is lenient only for *empty* non-key columns:** a NULL/blank non-key cell is allowed (the Python upserts insert NULLs freely and that data already lives in the DB) and becomes `null`. But a non-empty, malformed non-key cell **does hard-error** — e.g. a non-numeric value like `N/A` in a number column is reported (`Column "<label>": "<raw>" is not a number`) rather than silently coerced to null and dropped. Other hard errors: malformed format (a Week not matching `WW/YYYY`) and missing conflict-key columns. Missing/empty conflict-key rows are skipped, not errored.
