...
- **2026-05-28 (mobile tile redesign — vertical layout)** `[mobile-only]`: `MobileHomeIconTile` default variant switched from horizontal (icon-left + label-right, 88px tall, 2-line clamp) to vertical (icon-on-top 40×40 + label below centered, 116px tall, 3-line clamp, 13.5px font). Triggered by truncation of "Brazil Production Summary" (and other long names) at 360–426px viewports — horizontal layout only afforded ~100px for the label. Vertical layout hands the full tile width (~150–190px) to the label. Verified at 360/390/426 viewports: all 10 mobile-eligible tiles render with no ellipsis (DOM inspection — `scrollHeight === clientHeight`). Compact variant (used by Last-visited row, currently mounted nowhere on `/home`) retains horizontal layout. Desktop `/home` untouched.
