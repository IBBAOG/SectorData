...
- **Sidebar**: BrandLogo + **Period slider only** + the note "Net = field daily production × the company's effective stake (contract-tranche blend, monthly). Field labels show the latest month's stake." (copy updated 2026-06-18 with the blended-stake semantics). The level multi-selects moved out of the sidebar into the Explore surface; the 4-pill granularity toggle and the `CompanySelector` (featured pills + `<select>`) were **deleted**.
...
- Page heading "Daily Production" / "Net production by field, weighted by the company's effective stake" (MobileTopBar provided by MobileShell — NOT imported here). A footnote at the end of `CompanyMobileContent` mirrors the desktop sidebar's blended-stake methodology note (binding sync, 2026-06-18).
...
#### Canonical field merge: PITANGOLA → PEREGRINO (2026-06-08)

Por decisão do CEO, **PEREGRINO inclui PITANGOLA**: investidores tratam Peregrino = Peregrino + Pitangola (ambos PRIO 80%, mesma bacia `Campos`). O merge é feito **no SQL layer** via o mecanismo canônico do `/well-by-well` — `public.canonical_field_name(text)` + uma linha `('PITANGOLA','PEREGRINO','manual')` em `public.field_canonical_names` (migration `20260614000000`). **Não houve mudança de frontend** — as duas Views consomem o hook, que recebe o que as RPCs retornarem (uma única série "Peregrino" já somada).

Como `anp_cdp_diaria` tem linhas separadas de PEREGRINO e PITANGOLA por dia, é um relabel-**e-agregação**: as RPCs **field-level** (`get_anp_cdp_diaria_serie`, `get_anp_cdp_diaria_filtros`) e **company-level** (`get_anp_cdp_diaria_empresa_serie`, `get_anp_cdp_diaria_empresa_campos`) agora projetam `canonical_field_name(campo) AS campo`, somam (`SUM(petroleo_bbl_dia)`, `SUM(gas_mm3_dia)`, e os `*_net` no company-level) e agrupam pelo nome canônico. O filtro `p_campos` casa pelo **nome canônico** — selecionar `'PEREGRINO'` puxa AS DUAS linhas brutas (Peregrino + Pitangola). O selector (`get_anp_cdp_diaria_filtros.campos`) mostra `PEREGRINO` uma vez e **não** mostra `PITANGOLA`.

**Escopo do merge = field/company.** As RPCs de **Installation / Well** (`get_anp_cdp_diaria_instalacao_*`, `get_anp_cdp_diaria_poco_*`) **NÃO** foram tocadas — chaveiam por instalação/poço, não por campo. `get_anp_cdp_diaria_export_count` (oráculo de tamanho do export, nível bruto) e `get_anp_cdp_diaria_empresas` (contador de empresas elegíveis, morto no frontend) também ficaram intactos.

Exemplo confirmado (PRIO, validação 2026-06-08, dia 2026-06-02): antes do merge a série retornava **2 linhas** — `PEREGRINO` net oil 72.573,92 + `PITANGOLA` net oil 3.421,392. Depois do merge retorna **1 linha** `PEREGRINO` com net oil **75.995,312** bbl/dia (= 72.573,92 + 3.421,392, exato), net gas **153,324** Mm³/dia (= 143,5832 + 9,7408), bruta oil 94.994,2 e bruta gas 191,655. Filtrar `get_anp_cdp_diaria_serie(ARRAY['PEREGRINO'],…)` traz a mesma linha somada.

Exemplo confirmado (PRIO, smoke test 2026-06-05; **PEREGRINO já inclui Pitangola desde 2026-06-08**): Frade/Polvo/Tubarão Martelo (100%), Albacora Leste (90%), **Peregrino (80%, = Peregrino + Pitangola)** — todos com dado diário; **Wahoo (100%) — no daily data yet** (FPSO ainda fora do feed). Net Oil avg ≈ 157,5 kbpd (valor pré-merge; após o merge o net oil de Peregrino agrega Pitangola). A cobertura de PRIO passou de 6 linhas (com Pitangola separada já agregada no net via INNER JOIN) para **6 campos canônicos** sem `PITANGOLA` listada à parte.
...
#### Blended effective stakes (2026-06-18 — migration `20260618000000`)

**Bug fixed (confirmed, reconciled twice):** `field_stakes` carries contract-tranche rows as separate raw names (Petrobras: BÚZIOS 100% + BÚZIOS_ECO 85%, ATAPU 100% + ATAPU_ECO 52.5%, SÉPIA 100% + SÉPIA_ECO 30%), but the daily Power BI panel publishes ONE merged row per field — `*_ECO` names never appear there. The old raw-name join matched only the 100% ToR row and weighted the entire merged field at 100%, overstating Petrobras by **~185 kbpd** (Apr-2026: 2808.3 kbpd shown vs ≈2623.5 correct).

**Fix (DB-side, signatures unchanged):** `get_anp_cdp_diaria_empresa_serie` and `get_anp_cdp_diaria_empresa_campos` now return a per-month **effective blended stake** in `stake_pct` — the production-weighted blend of the field's tranches per `(canonical field, company, month)`, read from **`mv_production_monthly`** (the `/well-by-well` MV, column `stake_pct_weighted`) and carried forward to months the monthly CDP hasn't published yet. Fields with no blend fall back to the raw-name stake (byte-identical to the previous behavior).

Consequences the frontend relies on:

- **`stake_pct` varies by month** for blended fields (TUPI 67.348 Mar-26 → 67.311 Apr-26; BÚZIOS ≈ 88.918, ATAPU ≈ 71.262, SÉPIA ≈ 51.909 in Apr-26). The hook therefore **pins one display stake per field** — `latestStakeByCampo(rows)` (the stake of the field's latest reported day = the latest month's blend) — and EVERY label/series-key surface uses it: `projectCompany` dimensions (line-chart legend), `buildCompanyMonthlyOilByField` stack labels, `buildCompanyDailyOilMatrix` column headers, `buildCompanyFieldAggregates.stakePct` (mobile stake badges) and the coverage note (whose RPC already returns the latest blend). Keying labels on the per-row stake would fragment a field into one legend entry/trace/column per month. The underlying NET values keep their true per-month weighting (computed server-side); only the LABEL is pinned.
- **Blended fields emit exactly 1 row per (data, campo).** The old TUPI behavior (2 rows/day: a 65% group + a canonical-merged 100% group) is gone. The hook's per-field averages now divide by **distinct reporting days** (day-`Set`s in `orderCompanyFieldDims`, `buildCompanyFieldAggregates`, `buildCompanyMonthlyOilByField`) instead of raw row counts — the old row-count denominator would have halved TUPI's per-field average (60 rows over a 30-day month). Day count == row count for blended fields; the day-set also keeps the math correct for the theoretical no-blend fallback that may still emit per-stake-group rows (the serie GROUP BY includes `COALESCE(blend, raw stake)`).
- **New DB dependency:** the two company RPCs read `mv_production_monthly` (columns `canonical`, `empresa`, `ano`, `mes`, `oil_bbl_dia`, `stake_pct_weighted`). `/well-by-well` owners must not drop/rename the MV without updating these RPCs (note in the migration header). Blends move when `refresh_mv_production()` runs (pg_cron after the monthly ETL); the carry-forward absorbs the refresh lag.
- **Copy:** sidebar note (desktop) / methodology footnote (mobile) read "Net = field daily production × the company's effective stake (contract-tranche blend, monthly). Field labels show the latest month's stake."; chart/matrix captions say "weighted by effective stake"; matrix column headers carry a tooltip explaining the label stake is the latest loaded month's blend.

**Expected genuine gap vs `/well-by-well` (~75–90 kbpd, NOT a bug):** after the fix, `/anp-cdp-diaria` (Petrobras) still reads ~75–90 kbpd BELOW the `/well-by-well` stake-weighted monthly numbers for overlapping months. This is a SOURCE limitation of ANP's daily Power BI panel roster, not a weighting error: the daily feed is missing **2 new Búzios wells (7-BUZ-79-RJS and 7-BUZ-94D-RJS)** plus **~19 small onshore fields** that the monthly CDP form does capture. Readers comparing the two dashboards should expect the daily panel to undershoot by roughly that amount until ANP adds those wells/fields to the panel.

Smoke-test reference numbers (Petrobras, Apr-2026 monthly bar): total ≈ **2623.6 kbpd** (was 2808.3 pre-fix); BÚZIOS ≈ 732.5, TUPI ≈ 613.6, MERO ≈ 278.2, JUBARTE ≈ 206.5, ATAPU ≈ 84.7, SÉPIA ≈ 64.0. PRIO is unaffected (no multi-tranche fields).

#### TEMPORARY P-78 coverage banner (2026-06-10 — remove when ANP adds P-78)

The "Expected genuine gap" note above now has a **user-facing surface**: a prominent amber warning banner rendered by BOTH views. Same root cause as the residual gap: ANP's daily Power BI panel has not registered FPSO P-78 ("PETROBRAS 78") at ANY granularity (field totals, installations, wells) — verified live against ANP's API on 2026-06-10. Its two producer wells, 7-BUZ-79-RJS (first oil late Dec-2025) and 7-BUZ-94D-RJS (first oil late Mar-2026), Búzios, did ~87 kbpd gross in Apr-2026 and are ramping up.

- **VISIBILITY RULE (user decision 2026-06-10): Petrobras tab ONLY.** The banner renders only when the active company tab is Petrobras (`selectedEmpresa === PETROBRAS`, the type-checked constant exported by `useAnpCdpDiariaData.ts` next to `FIXED_COMPANIES`). It does NOT appear on the PRIO tab, on the desktop "Explore raw data" surface, or in the mobile Explore BottomSheet — P-78 is a Petrobras/Búzios asset, so the understatement only affects the Petrobras net view. No RPC/data-driven gating beyond that (the canary remains the removal trigger).
- **Component (single source of copy for both views):** `src/app/(dashboard)/anp-cdp-diaria/P78CoverageNotice.tsx` — amber alert (`role="alert"`, `#fff3cd` bg / `#ffe69c` border / `#d97706` left-accent + triangle icon / `#7d5800` text; same warning family as the `/well-by-well` tab-error boxes).
- **Desktop placement:** inside the Petrobras tab content — **below the primary tab bar, above the charts** (prominent, first element of the company content).
- **Mobile placement:** alert card right below the page heading / hero company toggle (above the sticky period pills), only while Petrobras is the selected company. The former repeat at the top of the Explore BottomSheet was removed with the Petrobras-only rule (Petrobras isn't the selected company on that surface).
- **Copy (title):** "FPSO P-78 not yet in ANP's daily panel"
- **Copy (body):** "ANP's daily panel does not yet include FPSO P-78 (Búzios). Its two producer wells (~87 kbpd gross in Apr-2026, ramping up) are absent at every level, so daily figures here understate Búzios and Petrobras net production by roughly 85–90 kbpd. Monthly data on Brazil Production Summary already includes P-78." ("Brazil Production Summary" links to `/well-by-well`. The auto-backfill-on-ANP-add fact intentionally stays out of the user-facing copy — it lives in the REMOVAL TRIGGER below and the component's code comment.)
- **REMOVAL TRIGGER:** the day ANP registers P-78 in the daily panel, the ops canary (`cdp_roster_canary.yml`) emails the team and the 3×/day full-range ETL auto-backfills the entire history on its own — then delete `P78CoverageNotice.tsx`, its two render sites (desktop Petrobras-tab slot + mobile landing card) and the `PETROBRAS` gating constant in `useAnpCdpDiariaData.ts`. No data work needed; also retire this section and fold the "Expected genuine gap" note (the residual gap will shrink to the ~19 small onshore fields only).
...
|---|---|---|
| `rpcGetAnpCdpDiariaFiltros` | `get_anp_cdp_diaria_filtros()` | `{ campos[], data_min, data_max }` (backend also returns `bacias[]`; dropped by wrapper since 2026-05-28). **`campos[]` é a lista canônica desde 2026-06-08** — `PEREGRINO` uma vez, sem `PITANGOLA` |
| `rpcGetAnpCdpDiariaSerie` | `get_anp_cdp_diaria_serie(p_campos, p_bacias, p_data_inicio, p_data_fim)` | `Array<{ data, campo, bacia, petroleo_bbl_dia, gas_mm3_dia }>` — wrapper pins `p_bacias = NULL` since 2026-05-28. **Agrega por `canonical_field_name(campo)` desde 2026-06-08**: 1 linha por (data, campo canônico, bacia), volumes somados; `p_campos` casa pelo nome canônico (`ARRAY['PEREGRINO']` traz Peregrino + Pitangola) |
...
| `rpcGetAnpCdpDiariaEmpresaSerie` | `get_anp_cdp_diaria_empresa_serie(p_empresa, p_data_inicio, p_data_fim)` | `Array<{ data, campo, bacia, stake_pct, petroleo_bbl_dia, gas_mm3_dia, petroleo_bbl_dia_net, gas_mm3_dia_net }>` — 1 linha por (data, **campo canônico**), INNER JOIN (campos sem dado diário não vêm). **Agrega por `canonical_field_name(campo)` desde 2026-06-08** — `campo` = nome canônico, volumes (bruto e net) **somados** entre variantes (PITANGOLA → PEREGRINO). **`stake_pct` é o blend mensal efetivo desde 2026-06-18** (migration `20260618000000`): blend production-weighted dos contract tranches do campo, lido de `mv_production_monthly`, com carry-forward para meses ainda sem CDP mensal — varia por mês (ex.: TUPI 67.348 Mar-26 → 67.311 Apr-26; BÚZIOS ≈ 88.918 Apr-26). Campos com blend emitem exatamente **1 linha/(data, campo)** (o antigo TUPI 2-linhas/dia acabou); só o fallback sem blend pode ainda emitir linhas por stake-group |
| `rpcGetAnpCdpDiariaEmpresaCampos` | `get_anp_cdp_diaria_empresa_campos(p_empresa)` | `Array<{ campo, stake_pct, has_daily_data }>` (ordenado has_daily_data DESC, stake DESC) — cobertura do stake. **Campos canônicos desde 2026-06-08**: `PEREGRINO` aparece uma vez (sem `PITANGOLA`); `has_daily_data` é `bool_or` entre variantes. **`stake_pct` desde 2026-06-18** = blend mensal mais recente disponível (fallback `MAX(stake raw)` quando o campo não tem blend) — consistente com os labels da série |
...
**Why instalacao/poco moved to DO UPDATE (2026-06-10):** required so a full-range re-run self-heals the historical damage from (a) the DSR Ø null-mask parser bug at the installation level (garbage `campo` values + mis-assigned measures) and (b) the `v_instalacoes_final` join at the well level, which intermittently dropped entire well-days (the well query is now join-free). With DO NOTHING those corrupted/missing rows would persist forever. The instalacao path also runs an idempotent `purge_instalacao_garbage()` before upserting — it executes on **every** pipeline run (not one-shot) and will keep running until explicitly removed from the extractor — deleting zombie rows keyed by numeric garbage installation names left behind by the pre-fix parser.
...
- `2026-06-07` — **Polish pós-QA (3 nits): fonte única do top-6 + contagem honesta de campos no subtítulo mobile.** (Nit 1, user-facing) O subtítulo dos `ChartCard` de empresa no mobile usava `companyFieldAggregates.length` — **capado em 7** (6 + Others) — então a Petrobras mostrava "total · 7 fields" quando há **37** campos com dado diário. Corrigido: novo `companyFieldCount` exposto pelo hook (= `companyBuckets.totalFieldCount` = top + others) → subtítulo agora "total · 37 fields" (Petrobras) / "total · 6 fields" (PRIO), honesto. (Nits 2+3, robustez) A decisão de "quais 6" agora vive num **único** lugar: novo factory `buildCompanyBuckets(companySerieRows)` → `CompanyBuckets` (memo `companyBuckets` no hook), que computa **uma vez** a ordem canônica (`orderCompanyFieldDims` por net oil avg), o split top-6+Others (`companyDisplayBuckets`), o `topSet`/`topCamposSet`, o `colorMap`, os mapas label↔campo e o `totalFieldCount`. **Todos os 4 surfaces consomem esse objeto**: `buildCompanyChart`, `buildCompanyMonthlyOilByField` e `buildCompanyDailyOilMatrix` agora recebem `buckets` (em vez de re-rodar `orderCompanyFieldDims`/`companyDisplayBuckets` internamente) e roteiam o fold de Others pelo `bucketOf` compartilhado (matriz em campo-space, barra/linha em label-space) — não mais `topSet.has(...)` inline; `capCompanyFieldAggregates(aggs, buckets)` reusa o `orderedCampos`/`topCamposSet` em vez de re-ordenar por `avgOilNet` por conta própria. **Comportamento idêntico** (mesmos 6 + Others hoje, totais preservados) — só removeu o acoplamento "consistente por convenção". Smoke test (Preview MCP, eval, landing PRIO anônimo): PRIO → 6 campos, **sem** Others nos 3 surfaces; Petrobras → BÚZIOS/TUPI/MERO/JUBARTE/ITAPU/SÉPIA + **Others (31)** (= 37 campos), barra de Maio **2.781,9 kbpd** (total preservado). `tsc`/`eslint` clean. Commit `refactor(anp-cdp-diaria): single source for top-6 selection + fix mobile field-count subtitle (QA nits)`.
- `2026-06-08` — **PEREGRINO inclui PITANGOLA (canonical field merge — field + company series + filtro)** (decisão do CEO: investidores tratam Peregrino = Peregrino + Pitangola; ambos PRIO 80%, mesma bacia `Campos`). Mudança **pura de RPC-layer** — **sem mudança de frontend** (as duas Views consomem o hook compartilhado, que recebe o que as RPCs retornam). Reusa o mecanismo canônico do `/well-by-well`: seed de `('PITANGOLA','PEREGRINO','manual')` em `field_canonical_names` (idempotente, `ON CONFLICT (variant) DO UPDATE`) → `canonical_field_name('PITANGOLA')='PEREGRINO'`. Como `anp_cdp_diaria` tem linhas separadas por dia, é relabel-**e-agregação**: 4 RPCs DROP/REPLACE (assinatura, `SECURITY DEFINER`, `search_path` e GRANTs idênticos) projetando `canonical_field_name(campo)` + `SUM(...)` + GROUP BY canônico — `get_anp_cdp_diaria_serie` (field, filtro `p_campos` casa pelo canônico → `ARRAY['PEREGRINO']` puxa Peregrino+Pitangola), `get_anp_cdp_diaria_filtros` (campos canônicos, `PEREGRINO` uma vez sem `PITANGOLA`), `get_anp_cdp_diaria_empresa_serie` (company, soma bruto + net, GROUP BY `data, canônico, bacia, stake_pct`), `get_anp_cdp_diaria_empresa_campos` (cobertura, `bool_or(has_daily_data)`, `MAX(stake_pct)`). **Installation/Well intactos** (chaveiam por instalação/poço, não campo); `get_anp_cdp_diaria_export_count` e `get_anp_cdp_diaria_empresas` intactos. Migration `20260614000000_anp_cdp_diaria_merge_pitangola_into_peregrino.sql`. **Cross-check (Regra F)**, PRIO 2026-06-02: antes 2 linhas (PEREGRINO net oil 72.573,92 + PITANGOLA 3.421,392); depois 1 linha PEREGRINO net oil **75.995,312** (= soma exata), net gas 153,324 (= 143,5832 + 9,7408). Filtros: `campos` tem PEREGRINO, **não** tem PITANGOLA. Cobertura PRIO: 6 campos canônicos, WAHOO ainda `false`. Grants/`SECURITY DEFINER`/`search_path` confirmados nas 4 RPCs.
- `2026-06-10` — **TEMPORARY P-78 coverage banner (both views, same commit — binding sync).** New shared component `P78CoverageNotice.tsx` (single source of copy) rendered: desktop below `DashboardHeader` / above the primary tab bar (visible on company tabs + Explore); mobile below the page heading (above the sticky period pills) + repeated at the top of the Explore BottomSheet. Static (no RPC gating); removal trigger = ANP registers P-78 in the daily panel → `cdp_roster_canary.yml` ops email → delete the component + render sites (ETL auto-backfills history). See "TEMPORARY P-78 coverage banner" section above. Also fixed stale upload-semantics wording: `purge_instalacao_garbage()` runs on EVERY pipeline execution (not one-shot). **Same-day update (user decision 2026-06-10): banner restricted to the Petrobras tab ONLY** — gated on `selectedEmpresa === PETROBRAS` (new type-checked constant exported by the hook next to `FIXED_COMPANIES`); desktop banner moved from above the tab bar into the Petrobras tab content (below the tab bar, above the charts); mobile landing card now conditional on Petrobras and the Explore-BottomSheet repeat removed. Component untouched (copy/styling/TEMPORARY comment). Both views, same commit (binding sync).
- `2026-06-18` — **Blended effective stakes (frontend adaptation)** — DB fix `20260618000000` (worker_supabase, already live) made `stake_pct` a per-month blended effective stake in the two company RPCs (see "Blended effective stakes" section above; Petrobras was overstated ~185 kbpd by the raw-name `field_stakes` join). Frontend changes (hook + both views, same commit): (1) new `latestStakeByCampo(rows)` helper pins ONE display stake per field (latest reported day's = latest month's blend) and is consumed by every label/series-key surface — `projectCompany`, `buildCompanyMonthlyOilByField`, `buildCompanyDailyOilMatrix`, `buildCompanyFieldAggregates` — eliminating the legend-fragmentation hazard (one field → N legend entries when monthly blends round differently); (2) per-field averages now divide by distinct reporting days (day-`Set`s) instead of row counts in `orderCompanyFieldDims` / `buildCompanyFieldAggregates` / `buildCompanyMonthlyOilByField` — removes the obsolete tolerance for TUPI's old 2-rows/day stake groups (row-count denominator halved its average) and stays correct for the theoretical no-blend fallback that may still emit per-stake-group rows (same-day fallback rows also sum into the latest-day net); (3) copy updated in BOTH views: desktop sidebar note + mobile methodology footnote ("Net = field daily production × the company's effective stake (contract-tranche blend, monthly). Field labels show the latest month's stake."), mobile page subtitle, company `headerSub`, chart/matrix captions ("weighted by effective stake"), matrix header tooltip; (4) rpc.ts wrapper docs updated (semantics only — signatures/types unchanged). Documented the new `mv_production_monthly` dependency and the genuine ~75–90 kbpd source shortfall vs `/well-by-well` (daily panel roster missing 7-BUZ-79-RJS, 7-BUZ-94D-RJS + ~19 small onshore fields). Smoke targets: Petrobras Apr-26 bar ≈ 2623.6 kbpd, BÚZIOS (~88.9%), ATAPU (~71.3%), SÉPIA (~51.9%); PRIO unchanged. `tsc` / `eslint` / vitest clean.
