...
- **Desktop placement:** directly below `DashboardHeader`, **above the primary tab bar** — visible on PRIO, Petrobras AND Explore raw data.
- **Mobile placement:** alert card right below the page heading (above the sticky period pills), **repeated at the top of the Explore BottomSheet** (the sheet covers the landing card).
...
- **REMOVAL TRIGGER:** the day ANP registers P-78 in the daily panel, the ops canary (`cdp_roster_canary.yml`) emails the team and the 3×/day full-range ETL auto-backfills the entire history on its own — then delete `P78CoverageNotice.tsx` and its render sites (desktop header slot + mobile landing card + Explore sheet top). No data work needed; also retire this section and fold the "Expected genuine gap" note (the residual gap will shrink to the ~19 small onshore fields only).
