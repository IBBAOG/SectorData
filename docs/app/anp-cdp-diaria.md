...
- **Copy (title):** "Data coverage notice — FPSO P-78 not yet in ANP's daily panel"
- **Copy (body):** "ANP's daily panel does not yet include FPSO P-78 (Búzios) — its two producer wells (~87 kbpd gross in Apr-2026, ramping up) are absent at every level, so daily figures here understate Búzios and Petrobras net production by roughly 85–90 kbpd. Monthly data on Brazil Production Summary already includes P-78; the full history here backfills automatically the moment ANP adds it." ("Brazil Production Summary" links to `/well-by-well`.)
