...
| `stock_guide_companies`, `stock_guide_drivers`, `stock_guide_sensitivities`, `stock_guide_scenario_grid`, `stock_guide_config` (`stock_guide_sensitivity` dormante) | dash-stock-guide (read via SECURITY DEFINER RPCs) + dash-admin (write via `is_admin()`-guarded RPCs) + Dados Locais (write em `stock_guide_scenario_grid` via service role) | Admin via RPCs (`stock_guide_scenario_grid` via service role). RLS habilitada **sem policies** em todas (hide-aware reads only). `stock_guide_companies` carrega os fundamentals (`net_debt_y1/y2`, `ebitda_y1/y2`, `net_income_y1/y2`, opcional `net_income_ex_y1` / `net_income_ex_y2 numeric` NULLABLE (BRL mn; lucro líquido ajustado, denominador do P/E **só** da linha "ex-tax credit" — `?? net_income_yN` quando NULL; linha normal nunca usa; adicionadas em `20260623000000_stock_guide_net_income_ex_per_year.sql`. Caso de uso: Vibra — ajustar market cap E lucro líquido simultaneamente), opcional **por-ano-forward** `npv_tax_credit_y1` / `npv_tax_credit_y2 numeric` NULLABLE (BRL mn; NPV dos créditos tributários reconhecidos por ano) — **único mecanismo de tax credit**: quando `npv_tax_credit_y1 > 0` OU `npv_tax_credit_y2 > 0`, o frontend renderiza uma **linha companheira "{Empresa} ex-tax credit"** com base **por ano** — EV/EBITDA, P/E, FCFE yield e Div yield 26E sobre `market cap live − npv_tax_credit_y1`, 27E sobre `market cap live − npv_tax_credit_y2` (os dois anos podem diferir); Market cap exibido da linha = base y1 (TP/Recomm/Upside repetidos; linha normal intocada, sobre o market cap cru). Adicionadas por `20260622000000_stock_guide_npv_tax_credit_per_year.sql`, que **dropou o escalar `npv_tax_credit`** (nascido em `20260620000000_stock_guide_npv_tax_credit.sql`) fazendo split nas duas colunas por-ano. Histórico: `20260621000000_retire_mcap_adj.sql` dropou `mcap_adj_y1/y2` (mecanismo anterior, nascido em `20260610120000_stock_guide_mcap_adj.sql`, que substituíra o ainda mais antigo `net_income_adj_y1/y2` / "adjusted earnings P/E"). Dado existente (UGPA3): `npv_tax_credit_y1 = 2746.575` (herdado do escalar pela migration idempotente) / `npv_tax_credit_y2 = 2316.238` (restaurado do antigo `mcap_adj_y2`). As 3 RPCs `get_stock_guide_comps` / `admin_get_stock_guide_companies` / `admin_upsert_stock_guide_company` foram recriadas via DROP+CREATE (RETURNS TABLE / chaves jsonb passam de `npv_tax_credit` escalar para `_y1`/`_y2`); a `comps` reasserta `GRANT … TO anon, authenticated`, e as 2 admin reasserta `REVOKE ALL FROM PUBLIC` + **`REVOKE EXECUTE FROM anon`** (fecha o ACL default que o Supabase auto-concede a anon em toda função pública — QA `cfaf60df`) + `GRANT … TO authenticated`. Em `20260623000000` as mesmas 3 RPCs foram recriadas de novo via DROP+CREATE, adicionando `net_income_ex_y1` / `net_income_ex_y2` (logo após `net_income_y2`) à RETURNS TABLE de `get_stock_guide_comps` (hide-aware: NULL p/ não-admin em linha oculta) + `admin_get_stock_guide_companies`, e às chaves jsonb lidas/persistidas pelo upsert — superset estrito; grants/SECURITY DEFINER/search_path re-assertados igual. `fcfe_y1/y2`, `dividends_y1/y2`, `volumes_y1/y2`, opcional `model_url text` NULLABLE (URL livre para o modelo financeiro baixável da empresa — Excel hospedado externamente, ex. SharePoint/Drive; sem validação de formato; NULL/vazio = sem modelo — adicionada por `20260626000000_stock_guide_model_url.sql`, que recriou via DROP+CREATE as 3 RPCs `get_stock_guide_comps` (hide-aware, anon+authenticated) / `admin_get_stock_guide_companies` / `admin_upsert_stock_guide_company` adicionando `model_url` ao fim da RETURNS TABLE das duas leituras e às chaves jsonb lidas/persistidas pelo upsert com coerção `NULLIF(trim(...), '')` → vazio vira NULL; grants/SECURITY DEFINER/search_path re-assertados — pegadinha #18)) — multiplos derivados live no browser sobre o market cap cru. **Sensibilidade = malha de cenários multi-eixo × multi-métrica (1-D `20260612000000`, multi-eixo `20260618200000`, multi-métrica `20260619000000`, atual):** `stock_guide_sensitivities.definition jsonb` carrega um bloco **não-sensível** `grid` (`{ axes: [{driver_id, label, unit, tmin, tmax, tstep}] (1..3, ordem = x,y,z), outputs: [{key, mode, label}] }`, sem empresa, gravado verbatim) que marca a tabela como malha interpolada. v2 (`20260619000000`): cada eixo referencia **qualquer driver do registry** via `driver_id` (estático ou dinâmico) + faixa `tmin/tmax/tstep`; `outputs[]` (`mode ∈ upside|yield|pe|absolute`) substitui o `output` escalar legado — multi-métrica por tabela. Os valores por papel ficam na tabela relacional `stock_guide_scenario_grid(sensitivity_id bigint FK→stock_guide_sensitivities ON DELETE CASCADE, ticker text, metric text NOT NULL DEFAULT 'target_price', x_value numeric, y_value numeric, z_value numeric, primary_value numeric)`, PK 6-col `(sensitivity_id, ticker, metric, x_value, y_value, z_value)`, criada por `20260612000000_stock_guide_scenario_grid.sql` (1-D, PK 3-col), estendida para multi-eixo por `20260618200000_stock_guide_scenario_grid_multi_axis.sql` (ALTER + `y_value`/`z_value` `NOT NULL DEFAULT 0` + PK 3→5-col) e para multi-métrica por `20260619000000_stock_guide_scenario_grid_multi_metric.sql` (`ADD COLUMN metric … DEFAULT 'target_price'` backfilla o seed + uploads 1-métrica omitem a coluna; PK 5→6-col; preservando RLS/ownership/grants). Leitura via RPC `get_stock_guide_scenario_grid(p_sensitivity_id bigint, p_limit integer DEFAULT NULL, p_offset integer DEFAULT 0) → TABLE(ticker, metric, x_value, y_value, z_value, primary_value)` (recriada via DROP+CREATE em `20260619000000` por `metric` na `RETURNS TABLE`, e de novo em `20260624000000` para adicionar `p_limit`/`p_offset`; SECURITY DEFINER + `SET search_path` + GRANT anon/authenticated re-assertados a cada DROP por pegadinha #18; hide-aware — só tickers `is_visible` para não-admin, mesmo critério de `get_stock_guide_sensitivity_tables`). **Paginação (`20260624000000`):** PostgREST limita respostas SETOF ao `max-rows` do projeto (50.000); a malha de `sensitivity_id=18` tem 194.481 rows (7 métricas × 27.783) e era silenciosamente truncada a 50k na leitura → interpolação rodava em malha incompleta + o card de contagem do Admin mostrava 50.000. Fix: `LIMIT p_limit OFFSET p_offset` ao fim do `ORDER BY` determinístico `(ticker, metric, x_value, y_value, z_value)` (que torna limit/offset estáveis; `LIMIT NULL` = todas as rows). O wrapper frontend `rpcGetStockGuideScenarioGrid` agora pagina com `p_limit=40000` até uma página crua < 40k; o card de contagem do Admin migrou para `admin_count_stock_guide_scenario_grid` (`count.total`). Escrita: caminho **principal** = upload **no próprio Admin Panel** (browser) via 2 RPCs admin novas em `20260619100000_admin_replace_stock_guide_scenario_grid.sql` — `admin_replace_stock_guide_scenario_grid(p_sensitivity_id bigint, p_rows jsonb, p_first_chunk boolean) RETURNS integer` (replace-total **chunked**: o 1º chunk apaga todas as rows do `sensitivity_id` e os seguintes acumulam; guard `is_admin()` → 42501; valida array/sensitivity/campos; **NaN explicitamente rejeitado**; `ON CONFLICT` PK 6-col idempotente; `REVOKE … FROM PUBLIC` + `GRANT … TO authenticated`) e `admin_count_stock_guide_scenario_grid(p_sensitivity_id bigint) RETURNS TABLE(total bigint, by_metric jsonb)` (count de confirmação pós-upload). O parse ExcelJS + validações são feitos **client-side** (`src/lib/stockGuideGridUpload.ts`, paridade com o uploader Python) → relatório de erros/warnings → envio em chunks (~2000 rows) → confirmação por count. Caminho **fallback** (automação) = service role via `scripts/manual/stock_guide_brent_grid_upload.py` v2 (Excel multi-aba — 1 aba por output, coordenadas posicionais na ordem dos eixos — replace-total por `sensitivity_id` cobrindo todas as métricas, `on_conflict` 6-col, bypassa RLS). O template é baixado do Admin Panel (gerado no browser via ExcelJS); o gerador Python `make_brent_grid_template.py` ficou DEPRECADO (só 1-métrica offline). O frontend interpola a malha por `(ticker, métrica)` **multilinearmente** ao vivo (2^d cantos) → valor da métrica → output derivado (Target price→Upside, FCFE→FCFE yield, Dividends→Div yield, Net income→P/E). **Camada elástica `compose` substituída:** o bloco `definition.compose` (`20260611000000`) e o helper `_sg_strip_compose(jsonb,text[])` ficaram **dormentes** (não dropados; `_sg_strip_compose` segue sendo chamado por `get_stock_guide_sensitivity_tables` por defesa em profundidade, mas o frontend não renderiza mais `compose`). **Extensão de contrato `definition` (`20260625000000_stock_guide_sensitivity_panels.sql`, DML-only — sem mudança de schema; layout reformulado em `20260627000000`):** o `definition jsonb` ganhou duas chaves OPCIONAIS `panel` (`'brent' | 'margin'`) + `row_label` (string). As chaves são validadas client-side no mapper `mapSensitivityTable` (só `brent`/`margin` sobrevive em `panel`; string não-vazia em `row_label`), gravadas **verbatim** por `admin_upsert_stock_guide_sensitivity_table` e retornadas **verbatim** por `get_stock_guide_sensitivity_tables` (RPCs intocadas — extensão é jsonb-only; as chaves de contrato seguem inalteradas). A migration `20260625000000` é pura DML: tagga as tabelas ids 10-13 com `panel`/`row_label`, **seeda 6 drivers estáticos** em `stock_guide_drivers` ("Vibra/Ultrapar EBITDA margin 2Q26 / 2026E / 2027E", unit `'BRL/m³'`, `current_value` NULL — o admin digita) e **deleta o driver lixo id 4** ("EBITDA Margin 2027", BRL/bbl, não-referenciado). **Consumo (iteração 2, `20260627000000`):** o dashboard renderiza dois blocos consolidados sempre-visíveis ("Brent sensitivity" / "EBITDA margin sensitivity"; selection-independent — o click na linha de comps não dirige mais a seção). Cada bloco renderiza **UMA TABELA POR DRIVER** (título = `name` do driver, ex. "Avg. Brent 2026 (USD/bbl)"); dentro de cada tabela-por-driver, cada tabela taggeada vira uma **linha-banda cinza** (rótulo = `row_label ?? title`, ex. "FCFE Yield" / "Dividend Yield") seguida de **uma linha por empresa** no `row_axis` da tabela — **eixos multi-empresa agora são canônicos** (`20260627000000` expandiu as tabelas 10-13 para PETR4/PRIO3/RECV3/BRAV3 × cenários 50-150); um **único marker de interpolação por tabela-driver** (todas as linhas compartilham o driver). Células com valor tipado `null` renderizam **em branco**; "—" só para células com valor tipado ainda não-computável (live/derivado carregando). Placeholder até o admin preencher. **`panel` estendido a tabelas GRID + 1ª malha de margem (`20260629000000_stock_guide_margin_scenario_grid.sql`, DML-only — sem mudança de schema):** a chave opcional `definition.panel` (`'brent' | 'margin'`), antes só em tabelas estáticas, agora também tagga tabelas de malha de cenários (`definition.grid`). A migration: (1) **seeda 3 novos drivers estáticos COMPARTILHADOS** em `stock_guide_drivers` (id 13 'EBITDA margin 2Q26', id 12 'EBITDA margin 2H26', id 14 'EBITDA margin 2027+', unit `'BRL/m³'`, `current_value` NULL, `source` NULL — distintos dos drivers de margem por-empresa ids 6–11; idempotente por `name`); (2) insere 1 nova row de malha de cenários em `stock_guide_sensitivities` (id 19 'Target Price - EBITDA Margin Sensitivity', `companies = {UGPA3,VBBR3}`, `value_mode` absolute, `display_order` 100, `definition.panel='margin'`) espelhando a shape da id 18 — 3 eixos referenciando os 3 drivers novos por `driver_id` resolvido por nome (template tmin 50 / tmax 250 / tstep 10, BRL/m³) + os mesmos 7 `outputs` da malha Brent (idempotente por `title`); (3) taggea a malha Brent existente (id 18 'Target Price - Brent Sensitivity') com `panel='brent'` (idempotente — só se tem `grid` e ainda não tem `panel`). **Sem DDL, sem mudança de RPC:** `get_stock_guide_sensitivity_tables` retorna `definition` verbatim (já cobre a row nova) e `get_stock_guide_scenario_grid` é `sensitivity_id`-scoped (já cobre a id 19). A malha de pontos em `stock_guide_scenario_grid` **começa vazia** — o analista sobe depois via Admin Panel (`admin_replace_stock_guide_scenario_grid`); até lá o dashboard exibe seu empty state. Schema completo + contrato RPC em [`docs/app/stock-guide.md`](../app/stock-guide.md) e [`docs/master.md`](../master.md) § "Contrato /stock-guide". |
...
| `anp_cdp_diaria_instalacao` | (data, instalacao) | campo (NOT NULL), petroleo_bbl_dia, gas_mm3_dia. Sem coluna bacia — entidade Power BI `v_instalacoes_final` não expõe bacia. ~16.3k rows (93 instalações; range 2025-11-09 → presente). Populada em modo **upsert `ON CONFLICT DO UPDATE`** (self-healing, desde 2026-06-10 — era append-only `DO NOTHING`): re-runs full-range corrigem o dano histórico do bug de Ø-null-mask do parser (que corrompia `campo` e algumas measures); o pipeline também roda um purge idempotente de zombie rows com chave `instalacao` numérica deixadas pelo parser pré-fix. | `20260508120001_anp_cdp_diaria_levels.sql` | `scripts/extractors/anp_cdp_powerbi.py --level instalacao` |
| `anp_cdp_diaria_poco` | (data, poco) | campo (nullable), bacia (nullable), instalacao (nullable; adicionada em `20260508130001`), petroleo_bbl_dia, gas_mm3_dia. ~180.7k rows (1.219 poços; range 2025-11-09 → presente). Populada em modo **upsert `ON CONFLICT DO UPDATE`** (self-healing, desde 2026-06-10 — era append-only `DO NOTHING`): a query do extractor agora é join-free (o join com `v_instalacoes_final` derrubava well-days inteiros intermitentemente) e re-runs full-range repõem os dias perdidos. **Nota:** atribuição poço↔campo é 1:1 (último mapeamento contratual). Para análise N:N (poços compartilhados entre múltiplos campos), use `anp_cdp_producao` (mensal × poço × campo, PK composta suporta N:N nativamente). Ver limitação documentada em [`docs/app/anp-cdp-diaria.md`](../app/anp-cdp-diaria.md). | `20260508120001_anp_cdp_diaria_levels.sql` + `20260508130001` (add instalacao) | `scripts/extractors/anp_cdp_powerbi.py --level poco` |
