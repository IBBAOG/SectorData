...
| `stock_guide_companies`, `stock_guide_drivers`, `stock_guide_sensitivities`, `stock_guide_config` (`stock_guide_sensitivity` dormante) | dash-stock-guide (read via SECURITY DEFINER RPCs) + dash-admin (write via `is_admin()`-guarded RPCs) | Admin via RPCs. RLS habilitada **sem policies** (hide-aware reads only). `stock_guide_companies` carrega os fundamentals (`net_debt_y1/y2`, `ebitda_y1/y2`, `net_income_y1/y2`, opcional `net_income_adj_y1/y2` adicionado `20260610000000`, `fcfe_y1/y2`, `dividends_y1/y2`, `volumes_y1/y2`) — multiplos derivados live no browser. `stock_guide_sensitivities.definition jsonb` ganhou um bloco opcional `compose` (sensibilidade **elástica**, `20260611000000`): `{ output, driver_keys[], anchors{driver_key→nível}, base{ticker→TP}, by_company{ticker→{driver_key→slope}}, scenarios{} }` — `compose != null` marca a tabela como elástica e o browser compõe TP+upside ao vivo. **Hide-strip:** o RPC `get_stock_guide_sensitivity_tables` recriado em `20260611000000` chama o helper novo `_sg_strip_compose(jsonb,text[])` (`IMMUTABLE SECURITY DEFINER`, não granted a anon/authenticated) que remove os tickers ocultos de `compose.base` e `compose.by_company` (únicos campos per-ticker sensíveis; `anchors`/`driver_keys`/`scenarios`/`output` passam intactos) — em ambos os paths (com e sem eixo company), só para não-admin. Schema completo + contrato RPC em [`docs/app/stock-guide.md`](../app/stock-guide.md) e [`docs/master.md`](../master.md) § "Contrato /stock-guide". |
