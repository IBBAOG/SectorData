...
| `stock_guide_companies`, `stock_guide_drivers`, `stock_guide_sensitivities`, `stock_guide_scenario_grid`, `stock_guide_config` (`stock_guide_sensitivity` dormante) | dash-stock-guide (read via SECURITY DEFINER RPCs) + dash-admin (write via `is_admin()`-guarded RPCs) + Dados Locais (write em `stock_guide_scenario_grid` via service role) | Admin via RPCs (`stock_guide_scenario_grid` via service role). RLS habilitada **sem policies** em todas (hide-aware reads only). `stock_guide_companies` carrega os fundamentals (`net_debt_y1/y2`, `ebitda_y1/y2`, `net_income_y1/y2`, opcional `net_income_adj_y1/y2` adicionado `20260610000000`, `fcfe_y1/y2`, `dividends_y1/y2`, `volumes_y1/y2`) — multiplos derivados live no browser. **Sensibilidade = malha de cenários multi-eixo de Brent (1-D `20260612000000`, multi-eixo `20260618200000`, atual):** `stock_guide_sensitivities.definition jsonb` carrega um bloco **não-sensível** `grid` (`{ axes: [{driver_key, label, unit}] (1..3, ordem = x,y,z), output:"target_price" }`, sem empresa, gravado verbatim) que marca a tabela como malha de Brent interpolada; os valores por papel ficam na tabela relacional `stock_guide_scenario_grid(sensitivity_id bigint FK→stock_guide_sensitivities ON DELETE CASCADE, ticker text, x_value numeric, y_value numeric, z_value numeric, primary_value numeric)`, PK `(sensitivity_id, ticker, x_value, y_value, z_value)`, criada por `20260612000000_stock_guide_scenario_grid.sql` (1-D) e estendida para multi-eixo por `20260618200000_stock_guide_scenario_grid_multi_axis.sql` (ALTER + `y_value`/`z_value` `NOT NULL DEFAULT 0` — eixo não usado = 0 — + swap do PK 3→5-col, preservando RLS/ownership/grants). Leitura via RPC `get_stock_guide_scenario_grid(p_sensitivity_id bigint) → TABLE(ticker, x_value, y_value, z_value, primary_value)` (recriada via DROP+CREATE em `20260618200000` por mudança na `RETURNS TABLE`; SECURITY DEFINER + `SET search_path` + GRANT anon/authenticated re-assertados; hide-aware — só tickers `is_visible` para não-admin, mesmo critério de `get_stock_guide_sensitivity_tables`). Escrita via service role (`scripts/manual/stock_guide_brent_grid_upload.py`, formato LONG Cartesiano-completo, replace-total por `sensitivity_id`, bypassa RLS). O frontend interpola a malha por ticker **multilinearmente** ao vivo (2^d cantos) → target price + upside. **Camada elástica `compose` substituída:** o bloco `definition.compose` (`20260611000000`) e o helper `_sg_strip_compose(jsonb,text[])` ficaram **dormentes** (não dropados; `_sg_strip_compose` segue sendo chamado por `get_stock_guide_sensitivity_tables` por defesa em profundidade, mas o frontend não renderiza mais `compose`). Schema completo + contrato RPC em [`docs/app/stock-guide.md`](../app/stock-guide.md) e [`docs/master.md`](../master.md) § "Contrato /stock-guide". |
