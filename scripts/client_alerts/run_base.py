...
Behaviour:
    ...
    * Missing required env (SUPABASE_URL / service key / RESEND_API_KEY) prints a
      clear message and exits non-zero WITHOUT a stack trace.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if missing:
        logger.error(
            ...
            "Set them in the workflow env (SUPABASE_URL, a service key, RESEND_API_KEY) "
            "and re-run. Skipping.",
            ", ".join(missing),
        )
        return 1
