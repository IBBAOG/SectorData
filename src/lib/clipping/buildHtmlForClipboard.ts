// Clipboard-specific HTML builder for the IBBA Oil & Gas digest.
//
// Why this exists as a SEPARATE file (and not as `buildHtml.ts`):
// `buildHtml.ts` is consumed both by `buildEml.ts` (Download .eml — which
// already renders correctly in Outlook when opened as a message) AND by the
// Copy button in `ClippingModal.tsx`. The .eml path is known-good and must
// NOT be touched. The clipboard path has its own hostile environment
// (Chrome → CF_HTML → Word HTML normaliser) that needs the table + <div> +
// <font> + nested <span> stack documented below.
//
// IMPORTANT — Why this HTML looks like 1999:
//
// Goal: the HTML must render with our brand colors / fonts when the user
// pastes it from the clipboard into a fresh Outlook compose window.
//
...
// Local copy of esc(). `esc` is private to buildHtml.ts and we deliberately
// avoid exporting it from there to keep buildHtml.ts byte-identical to the
// pre-intervention version that buildEml.ts depends on.
...
export function buildHtmlForClipboard(items: ClippingItem[], d: Date): string {
  ...
  // Full document — kept as a complete document for the clipboard path. When
  // Chrome converts to CF_HTML the <html>/<head>/<body> wrapper is dropped
  // (only the fragment inside <body> survives, which is exactly the table).
  return (
