...
| `stock_guide_companies`, `stock_guide_sensitivity`, `stock_guide_config` | ticker / ticker / id=1 | `/stock-guide` equities comps (price-independent fundamentals in BRL mn — `net_debt_y1`/`net_debt_y2` (per forward year), `net_income_y1/y2` (reported), optional `mcap_adj_y1/y2` (per forward year, BRL mn — NPV of tax credits / non-operating assets subtracted from the live market cap, NULL = no adjustment), optional `npv_tax_credit` (single BRL mn value — NPV of recognized tax credits; added `20260620000000`), `fcfe_y1/y2`, `dividends_y1/y2`, `ebitda_y1/y2`, `volumes_y1/y2`; the 4 multiples EV/EBITDA · P/E · FCFE Yield · Div Yield are derived live in the browser from the Yahoo price, never stored — the market cap feeding all 4 multiples (and the Upside target price, via the y1 NPV) is the **adjusted** market cap `Market cap − mcap_adj_yN` when present, else the raw market cap; EV is forward per year: `EV(year) = Adjusted market cap(year) + Net Debt(year)`, P/E is `Adjusted market cap(year) ÷ net_income_yN` (reported earnings). When `npv_tax_credit > 0` the comps table also renders an **"{Company} ex-tax credit" companion row** with `mktcap_ex = live market cap − npv_tax_credit`, re-deriving EV/EBITDA · P/E · FCFE yield · Div yield off `mktcap_ex` (TP/Recomm/Upside repeated; normal row untouched; included in exports). It coexists with `mcap_adj_y1/y2` with **no double-counting** — the ex row branches off the raw market cap, not the per-year adjusted one (future consolidation pending analyst decision)) + global config; RLS-enabled with **no policies** (all reads via hide-aware SECURITY DEFINER RPCs). `stock_guide_sensitivity` (legacy per-company 2D grid) kept dormant since the sensitivity model went table-based (2026-06-06) |
