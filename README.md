...
| `stock_guide_drivers`, `stock_guide_sensitivities` | id / id | Redesigned sensitivity model (2026-06-06): `stock_guide_drivers` is the central registry of macro/assumption variables (`name`, `unit`, `current_value`, `source` — e.g. "Brent average 2026E", USD/bbl, 80; **`source` added 2026-06-01 — NULL = Static, admin types `current_value`; a catalog key = Dynamic, value computed live in the browser from the Yahoo proxy via `src/hooks/useMarketDrivers.ts` — catalog (6 metrics since 2026-06-11): `avg_brent_2026`/`avg_brent_2027`/`avg_brent_2028` USD/bbl (realized + forward curve; 2028 = spot-flat fallback when the curve doesn't reach it), `avg_fx_2026`/`avg_fx_2027`/`avg_fx_2028` BRL/USD (spot-flat approx)**); `stock_guide_sensitivities` holds first-class sensitivity tables (`title`, `value_mode` ∈ absolute/yield/pe/ev_ebitda/upside, `companies text[]`, `definition jsonb` with two axes — driver/company/year — plus `cells` + optional `cells_secondary`). Covers cross-company, single-company 2D, by-year and 1D; value modes derived live in the browser. **Scenario-grid tables (multi-axis since `20260618200000`):** `definition` may carry a non-sensitive `grid` block (`{axes: [{driver_key, label, unit}] (1..3, order = x,y,z), output}` — axis metadata only, names no company) that marks the table as a multi-axis Brent interpolation mesh; the per-company `(brent coords → target price)` values live in the separate `stock_guide_scenario_grid` table (below), read via `get_stock_guide_scenario_grid`. The `20260618200000` migration converted the legacy `{x_driver_key, x_label, x_unit, output}` 1-D shape to the `axes` block. **This superseded the linear `compose` elastic layer** (`definition.compose` + `_sg_strip_compose` are no longer rendered by the frontend, left dormant in the DB). RLS-enabled, **no policies** (reads via hide-aware SECURITY DEFINER RPC `get_stock_guide_sensitivity_tables`) |
| `stock_guide_scenario_grid` | (sensitivity_id, ticker, x_value, y_value, z_value) | Multi-axis (1–3 axes) Brent interpolation mesh for `/stock-guide` sensitivity (1-D `20260612000000`, multi-axis `20260618200000`). One row per `(sensitivity table, ticker, axis-coordinate combination)` carrying `primary_value` = target price (BRL/share) at that scenario. `x_value`/`y_value`/`z_value` are the per-axis Brent levels (order = the `definition.grid.axes` order); `y_value`/`z_value` are `numeric NOT NULL DEFAULT 0` (an unused axis is permanently 0). The frontend reads the per-ticker mesh and interpolates **multilinearly** (2^d corners) against the live axis levels. FK `sensitivity_id`→`stock_guide_sensitivities` `ON DELETE CASCADE`. RLS-enabled, **no policies** — reads via hide-aware SECURITY DEFINER RPC `get_stock_guide_scenario_grid(p_sensitivity_id)` → `(ticker, x_value, y_value, z_value, primary_value)` (visible tickers only for non-admins); writes service-role only via `scripts/manual/stock_guide_brent_grid_upload.py` (LONG-format Cartesian-complete replace-total snapshot per `sensitivity_id`) |
