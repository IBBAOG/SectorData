...
-- Baseline (Apr-26 Petrobras / 13-month window):
--   get_production_brazil_aggregate     2 822 ms
--   get_production_company_aggregate   12 243 ms   ← worst offender
--   get_production_top_fields             268 ms
--   get_production_by_installation        172 ms
--   get_production_yoy_table              493 ms
--   get_production_field_timeseries     7 013 ms   ← second worst
--   get_production_installation_timeseries 33 ms
--   ─────────────────────────────────────────────
--   TOTAL                              ~23.0 s
--
-- Root cause: 2.2 M-row `anp_cdp_producao` JOIN `field_stakes` + per-row call to
-- `canonical_field_name()` (which itself does a SELECT against field_canonical_names).
-- Every dashboard load redoes this work for the same input grain.
--
-- Solution: 3 materialized views, each pre-joined / pre-aggregated to the grain
-- the RPCs actually need:
--   mv_brazil_monthly                 (ano, mes, ambiente)             — RPC 1
--   mv_production_monthly             (canonical, empresa, ano, mes, ambiente) — RPCs 2,3,4,7
--   mv_production_installation_monthly(instalacao, empresa, ano, mes)  — RPCs 5,6
--
-- Refresh: REFRESH CONCURRENTLY via refresh_mv_production(). Recommended call
-- site is the end of the CDP ETL upload (scripts/pipelines/anp/cdp/02_upload.py).
-- Until that's wired, manual REFRESH after each ETL run keeps data fresh.
--
-- All RPCs remain SECURITY DEFINER SET search_path = public, pg_temp (Pegadinha
-- #18) because mv_* inherit RLS from underlying tables (anon would get empty).
...
-- Pre-joins anp_cdp_producao × field_stakes, applies stake math, groups by
-- canonical name. Only campos with SUM(stake_pct)=100 are included.
...
SELECT
  ...
  -- Weighted stake (production-weighted) — same formula as Round 4 RPC,
  -- pre-computed here so get_production_top_fields can read directly.
  COALESCE(
...
-- For get_production_by_installation + get_production_installation_timeseries.
...
-- ───── (4) Refresh function ─────────────────────────────────────────────────
-- Call site (recommended): scripts/pipelines/anp/cdp/02_upload.py — last step.
-- Until that's wired, ETL operator must REFRESH manually (or pg_cron 4h).
...
-- Lock down: revoke PUBLIC + Supabase's auto-grants to anon/authenticated,
-- then keep only service_role. Supabase auto-grants EXECUTE to anon/authd on
-- every new function in `public` (default schema-level grants), so an explicit
-- REVOKE is required — REVOKE FROM PUBLIC alone does not suffice. Without this
-- the function would be callable via PostgREST as `/rest/v1/rpc/refresh_mv_production`,
-- giving any visitor a DoS vector (REFRESH CONCURRENTLY holds EXCLUSIVE locks).
...
-- ───── (5) Rewrite the 5 production RPCs to read from MVs ───────────────────
-- All bodies reduced to thin wrappers — JOIN/aggregation/canonical math is
-- pre-baked. Filters are now index lookups instead of seq scans.
...
-- (5b) Company aggregate ─ read mv_production_monthly (group canonicals into ambiente totals)
...
-- (5c) Top fields ─ read mv_production_monthly (group canonicals across ambientes)
...
AS $$
  WITH agg AS (
    SELECT
      ...
      -- Weighted across ambientes by oil_bbl_dia within this (canonical, empresa, month).
      -- Falls back to AVG(stake_pct_weighted) if zero production.
      COALESCE(
...
-- (5e) Installation timeseries ─ read mv_production_installation_monthly
...
-- (5f) Field timeseries ─ read mv_production_monthly (canonical match across ambientes)
...
AS $$
  ...
  total_row AS (
    SELECT
      ...
      SUM(ytd_avg_kbpd)     AS ytd_avg_kbpd   -- Round 2 fix retained
    FROM per_ambiente
  )
  SELECT scope,
         current_kbpd,
...
-- ───── (6) GRANTs — re-apply explicitly (Pegadinha #18 doctrine) ────────────
-- CREATE OR REPLACE preserves grants, but we re-state them defensively.
...
-- ───── (7) MV permissions — locked down ─────────────────────────────────────
-- Do NOT grant SELECT on the MVs to anon/authenticated. The advisor flags this
-- as `materialized_view_in_api` (PostgREST exposes any granted view), creating
-- a redundant attack surface alongside the SECURITY DEFINER RPCs. Access path
-- is RPC-only; MVs stay private to the function definer's privileges.

-- ───── (8) Initial population (non-concurrent — MVs were empty on CREATE) ───
...
-- ───── (9) ANALYZE so planner has stats on day 1 ────────────────────────────
