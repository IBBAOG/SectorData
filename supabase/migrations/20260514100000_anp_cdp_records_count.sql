-- ANP CDP: extend get_anp_cdp_poco_serie to include records_count (COUNT(*))
-- alongside wells_count (COUNT(DISTINCT poco)) and fields_count (COUNT(DISTINCT campo)).
-- The ANP CDP portal paginates by record (well x installation), so records_count
-- reflects the raw row count the portal exposes (e.g. "1-25 of 774").
