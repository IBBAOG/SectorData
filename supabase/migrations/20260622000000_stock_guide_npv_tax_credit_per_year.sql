-- ============================================================================
-- Stock Guide — per-year NPV tax credit (BRL mn).
--
-- The analyst decided the NPV of the tax credit differs by horizon (2026 ≠ 2027).
-- Replace the single scalar npv_tax_credit (migration 20260620000000) with two
-- per-year columns: npv_tax_credit_y1 / npv_tax_credit_y2.
--
-- DB layer only. The browser ex-tax-credit companion row adjusts market cap by
-- the per-year NPV when re-deriving the per-year multiples (the consuming front
-- is updated separately; the front-old × DB-new window is tolerant — the JS
-- mapper reads the keys via toNumOrNull, so a missing key yields null and the
-- companion row simply doesn't render until the front catches up — no crash).
--
-- Order: (1) ADD the two new columns, (2) guarded data migration (idempotent,
-- re-applicable — DO/IF EXISTS + EXECUTE on a string so a re-run after the old
-- column is dropped never fails 42703 — same technique as fix 3cb595bc /
-- migration 20260621000000), (3) UGPA3 data restore, (4) DROP+CREATE the 3 RPCs
-- swapping the single field for the two per-year fields (re-asserting
-- SECURITY DEFINER + search_path + the CORRECT grants — pegadinha #18; comps:
-- anon+authenticated, admin RPCs: authenticated only — QA finding cfaf60df),
-- (5) DROP the old single column after the RPCs no longer reference it.
--
...
-- ---------------------------------------------------------------------------
-- (1) Add the two per-year columns (BRL mn; NULL = no ex-tax-credit row)
-- ---------------------------------------------------------------------------
ALTER TABLE public.stock_guide_companies
  ADD COLUMN IF NOT EXISTS npv_tax_credit_y1 numeric,   -- BRL mn, 2026E NPV
  ADD COLUMN IF NOT EXISTS npv_tax_credit_y2 numeric;   -- BRL mn, 2027E NPV

-- ---------------------------------------------------------------------------
-- (2) Data migration — idempotent. Seed y1 from the retired scalar npv_tax_credit
--     (the scalar previously stood in for both years). Only fill when the target
--     is still NULL so re-runs never clobber a real value. Guarded + EXECUTE so a
--     re-run of this file AFTER the old column is dropped (db push --include-all +
--     phantom revert — pegadinha #22) does not fail with 42703.
-- ---------------------------------------------------------------------------
DO $do$
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name  = 'npv_tax_credit'
  ) THEN
    EXECUTE $upd$
      UPDATE public.stock_guide_companies
      SET npv_tax_credit_y1 = COALESCE(npv_tax_credit_y1, npv_tax_credit)
      WHERE npv_tax_credit IS NOT NULL
        AND npv_tax_credit_y1 IS NULL
    $upd$;
  END IF;
END
$do$;
...
-- ---------------------------------------------------------------------------
-- (3) UGPA3 data restore — the 2027 NPV (y2) value lost when mcap_adj was retired
--     (UGPA3 mcap_adj_y2 = 2316.238, per the audit record). Idempotent: only fill
--     when still NULL. y1 (2746.575) is restored by step (2) from the scalar.
-- ---------------------------------------------------------------------------
UPDATE public.stock_guide_companies
SET npv_tax_credit_y2 = 2316.238
WHERE ticker = 'UGPA3'
  AND npv_tax_credit_y2 IS NULL;

-- ============================================================================
-- (4) Recreate the 3 RPCs swapping npv_tax_credit -> npv_tax_credit_y1/y2.
...
-- ---------------------------------------------------------------------------
-- (5) Drop the retired single column (after the RPCs no longer reference it).
-- ---------------------------------------------------------------------------
ALTER TABLE public.stock_guide_companies
  DROP COLUMN IF EXISTS npv_tax_credit;
