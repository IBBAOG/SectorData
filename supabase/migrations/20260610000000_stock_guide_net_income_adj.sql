-- ============================================================================
-- Stock Guide — adjusted net income (credit-adjusted) for the P/E numerator
--
-- New feature on /stock-guide: an optional per-company "adjusted net income"
-- (e.g. Vibra's net income adjusted by tax credits). When present, the browser
-- uses it as the P/E numerator instead of the raw net income:
--
--   P/E(year) = Market cap ÷ COALESCE(net_income_adj(year), net_income(year))
--
-- DB layer only. Two nullable columns are added to stock_guide_companies and
-- threaded through the three comps RPCs. NULL = no adjustment (front falls back
-- to the raw net_income_y1/y2). No values populated here — Eduardo enters them
-- via the admin panel.
--
-- RPCs rebuilt/extended (all retrocompatible — columns/keys ADDED, none removed):
...
------------------------------------------------------------
-- 1) Add the two nullable adjusted-net-income columns (BRL mn)
------------------------------------------------------------
ALTER TABLE public.stock_guide_companies
  ADD COLUMN IF NOT EXISTS net_income_adj_y1 numeric,   -- forward year 1 (2026E), BRL mn; NULL = no adjustment
  ADD COLUMN IF NOT EXISTS net_income_adj_y2 numeric;   -- forward year 2 (2027E), BRL mn; NULL = no adjustment
...
CREATE FUNCTION public.get_stock_guide_comps()
  RETURNS TABLE (
    ...
    net_income_adj_y1   numeric,
    net_income_adj_y2   numeric,
    fcfe_y1             numeric,
    fcfe_y2             numeric,
  ...
  AS $$
    SELECT
      ...
      CASE WHEN (c.is_visible OR public.is_admin()) THEN c.net_income_adj_y1  ELSE NULL END,
      CASE WHEN (c.is_visible OR public.is_admin()) THEN c.net_income_adj_y2  ELSE NULL END,
  ...
  BEGIN
    ...
    RETURN QUERY
      SELECT
        ...
        c.net_income_adj_y1, c.net_income_adj_y2,
...
CREATE OR REPLACE FUNCTION public.admin_upsert_stock_guide_company(
  ...
  BEGIN
    ...
    INSERT INTO public.stock_guide_companies (
      ...
      net_income_adj_y1, net_income_adj_y2,
      ...
      (p_data->>'net_income_adj_y1')::numeric,
      (p_data->>'net_income_adj_y2')::numeric,
    ...
    ON CONFLICT (ticker) DO UPDATE SET
      ...
      net_income_adj_y1  = EXCLUDED.net_income_adj_y1,
      net_income_adj_y2  = EXCLUDED.net_income_adj_y2,
